    ...
    public class EventDataservice : IEventDataservice
    {
        ...
        public EventDTO GetSpecificGroupEvent(int EventId, int GroupId)
        {
            ...
            using (MySqlConnection con = DatabaseConnection.CreateConnection())
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand("SELECT * FROM event WHERE (group_id = @group_id AND id = @id)", con))
                    {
        ...
        public void Post(EventDTO eventDTO)
        {
            using (MySqlConnection con = DatabaseConnection.CreateConnection())
                ...
                {
                    con.Open();
                    using (MySqlCommand cmd = new MySqlCommand("INSERT INTO event(start_time, Description,Title,group_id,picture_url) VALUES(@start_time, @Description,@Title,@group_id,@picture_url)", con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@group_id", eventDTO.GroupId);
